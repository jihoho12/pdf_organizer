...
/** 특정 날짜 폴더명 반환 */
function dateFolderName(dateStr: string): string {
  ...
  if (isNaN(d.getTime())) throw McpErrors.invalidParams(`올바르지 않은 날짜: ${dateStr}`);
...
/** Date 객체로 폴더명 반환 */
function dateToFolderName(date: Date): string {
...
function resolveDestPath(destDir: string, fileName: string): string {
...
const organizePdfTool = defineTool('organize_pdf', {
  description:
    'PDF 파일을 오늘 날짜(YYYY년 MM월 DD일) 폴더로 자동 정리합니다. ' +
...
const organizeByDateTool = defineTool('organize_by_file_date', {
  description:
    '디렉토리 안의 PDF 파일을 각 파일의 날짜(수정일 또는 생성일) 기준으로 ' +
  ...
  handler: async ({ source_dir, base_dir, date_type = 'modified', copy = false, recursive = false }) => {
    ...
    const results: { from: string; to: string; folder: string }[] = [];
